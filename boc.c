...
static void raw_on(void){
     ...
     * _setmode(BINARY) stops the CRT mangling multi-byte sequences
     * in text mode before they reach the console. */
    _setmode(_fileno(stdout), _O_BINARY);
    SetConsoleOutputCP(65001);
    SetConsoleCP(65001);
    /* Force a TrueType font with full box-drawing coverage.
     * The default raster/OEM font is missing many codepoints (╔╗╚╝╠╣…);
     * Consolas ships with Windows Vista+ and covers the entire Box Drawing
     * and Block Elements Unicode blocks. */
    {
        ...
        SetCurrentConsoleFontEx(ho, FALSE, &cfi);
    }
}
...
/* read_key — cross-platform single-keypress reader.
 *
...
typedef struct{int magic,version,used,turn,week,men_alive,overall_morale;
    ...
    static void screen_upgrades(void){
        ...
        for(;;){
            ...
            Key k=read_key(); if(k==KEY_ESC||k==KEY_Q) break;
    ...
    static int screen_save_load(int is_save){
        ...
        for(;;){
            ...
            Key k=read_key(); if(k==KEY_ESC) return -1;
    ...
    static Difficulty screen_difficulty(void){
        ...
        for(;;){
            ...
            Key k=read_key(); if(k==KEY_ESC) return DIFF_NORMAL;
    ...
    static int screen_pause_menu(void){
        ...
        for(;;){
            ...
            Key k=read_key(); if(k==KEY_ESC) return 0;
    ...
    static void screen_end(void){
        ...
        at(24,10);fg(GRY);fputs("Press any key...",stdout);attr_rst();fflush(stdout);read_key();
    }
